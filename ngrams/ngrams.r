...
total <- read.table(file = "/Users/iain/Desktop/culturomics/googlebooks-eng-all-totalcounts-20120701.txt", header=F, sep="\t", fill = T, quote = "", stringsAsFactors = F)


total_counts = data.frame(year=integer(), match=integer(), volume=integer(), last=integer())

n = apply(total, 1, function(y) { strsplit(y, ',') })
...
ggplot(data=years, aes(x=V2, y=V3, col = V1)) + 
  geom_line() + 
  xlab("Years") + 
  ylab("Frequency")
